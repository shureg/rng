/* Modified by Alexander Kabanov, Feb 2009. 
 * Changed the extension to .cc to comply with the local project convention. 
 * Also provided an enclosing namespace
 */
...
double GAUSSIAN_NUMERICS::CumulativeNormal(double x)
{
    ...
    if (x<-7.0)
        result = GAUSSIAN_NUMERICS::NormalDensity(x)/sqrt(1.+x*x);
    
    else 
    {
        if (x>7.0)
            result = 1.0 - GAUSSIAN_NUMERICS::CumulativeNormal(-x);
        else
        {
            double tmp = 1.0/(1.0+0.2316419*fabs(x));

            result=1-GAUSSIAN_NUMERICS::NormalDensity(x)*
